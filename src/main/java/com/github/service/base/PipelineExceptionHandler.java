  ...
  public void exceptionCaught(ChannelHandlerContext context, Throwable cause) {
    ...
    final FullHttpResponse response =
        new DefaultFullHttpResponse(HttpVersion.HTTP_1_1, HttpResponseStatus.INTERNAL_SERVER_ERROR,
            Unpooled.copiedBuffer("BaseService Error", CharsetUtil.UTF_8));
      ...
      public void operationComplete(ChannelFuture future) {
        if (!future.isSuccess()) {
          final FullHttpResponse response = new DefaultFullHttpResponse(HttpVersion.HTTP_1_1,
              ...
              Unpooled.copiedBuffer("BaseService Connection Error", CharsetUtil.UTF_8));
          response.headers().set(HttpHeaderNames.CONTENT_TYPE, "application/json");
          context.write(response).addListener(ChannelFutureListener.CLOSE);
        }
      }
    ...
    context.write(message, promise.addListener(new ChannelFutureListener() {
      ...
      public void operationComplete(ChannelFuture future) {
        if (!future.isSuccess()) {
          final FullHttpResponse response = new DefaultFullHttpResponse(HttpVersion.HTTP_1_1,
              HttpResponseStatus.INTERNAL_SERVER_ERROR,
              Unpooled.copiedBuffer("BaseService Write Error", CharsetUtil.UTF_8));
          response.headers().set(HttpHeaderNames.CONTENT_TYPE, "application/json");
